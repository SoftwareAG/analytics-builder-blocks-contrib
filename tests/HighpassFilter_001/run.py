...
class PySysTest(AnalyticsBuilderBaseTest):
	def execute(self):
		...
		self.modelId = self.createTestModel('apamax.analyticsbuilder.custom.HighpassFilter',{'cutoff':1.0, 'windowDurationSec':5.0})
	...
	def validate(self):
		...
		self.assertGrep('output.evt', expr=self.outputExpr('filtered', properties='.*"highpassFilter":any.*sequence.*float.*,.*0,0,0,0,1,1,1,1.*'), contains=False)
		self.assertGrep('output.evt', expr=self.outputExpr('filtered', properties='.*"highpassFilter":any.*sequence.*float.*,.*0,0,0,0,1,1,1.*'), contains=False)
		self.assertGrep('output.evt', expr=self.outputExpr('filtered', properties='.*"highpassFilter":any.*sequence.*float.*,.*0,0,0,0,1,1,1,2.*'), contains=False)
		self.assertGrep('output.evt', expr=self.outputExpr('filtered', properties='.*"highpassFilter":any.*sequence.*float.*,.*2.*'), contains=True)
		self.assertGrep('output.evt', expr=self.outputExpr('filtered', properties='.*"highpassFilter":any.*sequence.*float.*,.*0,1,2,3.*'), contains=False)
		self.assertGrep('output.evt', expr=self.outputExpr('filtered', properties='.*"highpassFilter":any.*sequence.*float.*,.*2,3.*'), contains=True)
